...
object TypeChecker extends SemanticChecker[(List[Option[Type]], VoidableType)] {
  ...
  def leave(node: Expr, st: SymbolTable, t: T): Result[T] = node match {
    ...
    case Expr.Call(call) => st.lookup(call.method.name) match {
      case Some(Descriptor.Method(FunctionType(_, VoidableType.Primitive(typ)))) => Result.Good(push(t, Some(typ)))
      case Some(Descriptor.Method(FunctionType(_, _))) => Result.Error(SemanticError(call.getSource, "If a method call is used as an expression, the method must return a result; found void method " + call.method.name), push(t, None))
      case _ => Result.Good(push(t, None))
    }
